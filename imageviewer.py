...
class ImageViewer(QMainWindow):
    def __init__(self, parent=None):
        ...
        # Option: use RNG for per-image/per-method application (threshold 0.5)
        rng_row = QHBoxLayout()
        self._use_rng_checkbox = QCheckBox("Use RNG (p=0.5)")
        # default: enabled
        self._use_rng_checkbox.setChecked(True)
        rng_row.addWidget(self._use_rng_checkbox)
        rng_row.addStretch()
        tab3_layout.addLayout(rng_row)
    ...
    def _create_dataset(self):
        ...
        # determine whether to use RNG per-image/per-method (checkbox)
        use_rng = True
        try:
            use_rng = bool(self._use_rng_checkbox.isChecked())
        except Exception:
            use_rng = True

        status, augmented_count = mgr.batch_augment(src, dst, selected_methods, use_rng)
