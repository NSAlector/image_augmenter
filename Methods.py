...
class AugmentationManager:
    ...
    def batch_augment(self, src_dir: str, dst_dir: str, methods: List[str], use_rng: bool = True) -> Tuple[str, int]:
        ...
        Для каждого изображения и каждого метода делается независимый бросок PRNG; метод
        применяется если `random.random() < 0.5` (порог 0.5).
        Возвращает кортеж `(status, saved_count)`.
        """
        ...
        for idx, (fpath, orig_img) in enumerate(originals):
            try:
                ...
                applied_names = []
                # применяем эффекты последовательно; для каждого метода при включенном use_rng
                # делается независимый бросок PRNG (порог 0.5). Если use_rng==False, применяем все методы.
                for m in methods:
                    if use_rng:
                        try:
                            r = random.random()
                            ...
                            r = 0.0
                        if r >= 0.5:
                            # пропускаем этот метод для данного изображения
                            continue
                ...
                # сохраняем итоговое изображение в staging (даже если applied_names пуст)
        ...
        for root, _, filenames in os.walk(staging_root):
            ...
            for f in filenames:
                ...
                # if target exists, generate unique name by appending _i before extension
                if os.path.exists(t):
